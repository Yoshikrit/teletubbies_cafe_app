...
public class PaymentMed extends javax.swing.JFrame {
    ...
    private int foodid;
    private GetOrder gO;
    private DateFormat dateformatforDB;
    private DateFormat dateformatforBill;
    private Date date;
...
//    private Main m;
    ...
    public PaymentMed(Order order ,GetOrder gO,Main m) {
        this.order= order;
        this.gO = gO;
//        this.m = m;
        initComponents();
        TotalPayment.setText("total payment : "+(double)order.getTotalPrice());
        qrTotalpayment.setText("total payment : "+(double)order.getTotalPrice());
        this.dateformatforDB = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss",Locale.ENGLISH);
        this.dateformatforBill = new SimpleDateFormat("dd-MM-yyyy HH-mm-ss",Locale.ENGLISH);
        this.date = new Date();
        this.dateforDB = dateformatforDB.format(date);
        String dateforBill = dateformatforBill.format(date);
        order.setOrderDate(dateforBill);
    ...
    private void totalPaymentPayButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_totalPaymentPayButtonActionPerformed
        try {
            ...
            double payment = Double.parseDouble(showPanelNumber.getText());
            if (Double.parseDouble(showPanelNumber.getText())  - order.getTotalPrice() >= 0  ){
             ...
             if (pay.getPaymentStatus()){
                ...
                for(int i = 0;i < order.getOrderFoodName().size();i++){
                    food = order.getFood().get(i);
                    foodid = food.getfoodID();
                    DatabaseHandler.insertOrder(new DatabaseHandler(),new SaleOrderDB(order.getOrderID(),order.getOrderQuantity().get(i),order.getTotalPrice(),order.getOrderStatus(),foodid,order.getEmpID(),Double.parseDouble(showPanelNumber.getText()),"cash",dateforDB));
                 }
                EmployeeDB emp = DatabaseHandler.findEmployeeById(new DatabaseHandler(), order.getEmpID());
                
                //add order into QueueCustomerPanel
                QueueCustomerPanel.addCookingTable(order);
                //add into Exchange Panel and open Exchange
                new Exchange(emp,pay,payment,gO).setVisible(true);
         ...
         try { 
            ...
            for(int i = 0;i < order.getOrderFoodName().size();i++){
                ...
                if (pay.getPaymentStatus()){
                    ...
                    KitchenWindow.addOrder(order);
                    //DatabaseHandler.insertOrder(new DatabaseHandler(),new SaleOrderDB(order.getOrderID(),order.getOrderQuantity().get(i),order.getTotalPrice(),order.getOrderStatus(),foodid,order.getEmpID(),Double.parseDouble(showPanelNumber.getText()),"cash",dateFormat.format(date)));
                    DatabaseHandler.insertOrder(new DatabaseHandler(),new SaleOrderDB(order.getOrderID(),order.getOrderQuantity().get(i),order.getTotalPrice(),order.getOrderStatus(),foodid,order.getEmpID(),Double.parseDouble(showPanelNumber.getText()),"scan", dateforDB));
            ...
            EmployeeDB emp = DatabaseHandler.findEmployeeById(new DatabaseHandler(), order.getEmpID());
            //print bill
            new Receipt(emp.getFirstName() + " " + emp.getLastName(),pay,gO,order.getTotalPrice() , 0 ).setVisible(true);
            //order plus one;
            GetOrder.plusOrder();
            //destroy panel
            dispose();
