...
public class EmployeeTable {
    ...
    public static int removeEmployeeByName(DatabaseHandler dbHandler, EmployeeDB emp) {
         String sql ="delete from teletubbies.Employee where FirstName = ? and LastName = ?";
         
         int rowDeleted;
         try {
            rowDeleted = dbHandler.update(sql, emp.getFirstName(), emp.getLastName());
         }
         catch (SQLException ex ) {
             rowDeleted = 0;
         }
    ...
    public static EmployeeDB findEmployeeByFullName(DatabaseHandler dbHandler, String firstName, String lastName) throws SQLException {
        String sql = "select * from teletubbies.Employee where FirstName = ? and LastName = ?";
        ResultSet rs;
        EmployeeDB emp = null;
        rs = dbHandler.query(sql, firstName, lastName);
        if (rs.next()) {
           emp = new EmployeeDB();
           emp.setEmployeeID(rs.getInt("EmployeeID"));
           emp.setFirstName(rs.getString("FirstName"));
